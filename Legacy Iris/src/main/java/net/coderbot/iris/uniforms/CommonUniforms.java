...
public final class CommonUniforms {
	...
	public static void addCommonUniforms(DynamicUniformHolder uniforms, IdMap idMap, PackDirectives directives, FrameUpdateNotifier updateNotifier) {
		...
		uniforms.uniform2i("atlasSize", () -> {
			...
			Vector2f atlasSize = TextureAtlasTracker.INSTANCE.getAtlasSize(glId);
	...
	static float getBlindness() {
		...
		if (cameraEntity instanceof LivingEntity) {
			EffectInstance blindness = ((LivingEntity) cameraEntity).getEffect(Effects.BLINDNESS);
	...
	private static float getPlayerMood() {
		...
		return ((ClientPlayerEntity)client.cameraEntity).getCurrentMood();
	}
	...
	private static Vector2f getEyeBrightness() {
		if (client.cameraEntity == null || client.level == null) {
			return Vector2f.ZERO;
		}

		Vector3d feet = client.cameraEntity.position();
		Vector3d eyes = new Vector3d(feet.x, client.cameraEntity.getEyeY(), feet.z);
		BlockPos eyeBlockPos = new BlockPos(eyes);

		int blockLight = client.level.getBrightness(LightType.BLOCK, eyeBlockPos);
		int skyLight = client.level.getBrightness(LightType.SKY, eyeBlockPos);

		return new Vector2f(blockLight * 16.0f, skyLight * 16.0f);
	}
	...
	private static float getNightVision() {
		...
		if (cameraEntity instanceof LivingEntity) {
			...
			if (livingEntity.getEffect(Effects.NIGHT_VISION) != null) {
		...
		if (client.player != null && client.player.hasEffect(Effects.CONDUIT_POWER)) {
