...
public class DeferredWorldRenderingPipeline implements WorldRenderingPipeline {
	...
	public DeferredWorldRenderingPipeline(ProgramSet programs) {
		...
		noise = programs.getPack().getCustomNoiseTexture().flatMap(texture -> {
			try {
				Texture customNoiseTexture = new NativeImageBackedCustomTexture(texture);
	...
	public static GbufferProgram getProgramForSheet(IParticleRenderType sheet) {
		if (sheet == IParticleRenderType.PARTICLE_SHEET_OPAQUE || sheet == IParticleRenderType.TERRAIN_SHEET || sheet == IParticleRenderType.CUSTOM) {
			return GbufferProgram.TEXTURED_LIT;
		} else if (sheet == IParticleRenderType.PARTICLE_SHEET_TRANSLUCENT) {
