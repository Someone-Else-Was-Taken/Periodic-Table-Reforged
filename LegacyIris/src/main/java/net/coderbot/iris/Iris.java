...
public class Iris {
	...
	public Iris() {
		...
		MinecraftForge.EVENT_BUS.addListener(this::handleKeybinds);
		MinecraftForge.EVENT_BUS.register(this);
	}

	public void onInitializeClient(final FMLClientSetupEvent event) {
		if (ModList.get().isLoaded("magnesium")) {
			ModContainer modContainer = ModList.get().getModContainerById("magnesium").get();
			sodiumInstalled = true;
			String versionString = modContainer.getModInfo().getVersion().toString();
			...
			//if (!versionString.contains(SODIUM_VERSION)) {
			//	sodiumInvalid = true;
			//}
		}
		...
		IRIS_VERSION = iris.getModInfo().getVersion().toString();

		//Disabled due to missing mixins
		//physicsModInstalled = ModList.get().isLoaded("physicsmod");
		...
		reloadKeybind = new KeyBinding("iris.keybind.reload", InputMappings.Type.KEYSYM, GLFW.GLFW_KEY_R, "iris.keybinds");
		toggleShadersKeybind = new KeyBinding("iris.keybind.toggleShaders", InputMappings.Type.KEYSYM, GLFW.GLFW_KEY_K, "iris.keybinds");
		shaderpackScreenKeybind = new KeyBinding("iris.keybind.shaderPackSelection", InputMappings.Type.KEYSYM, GLFW.GLFW_KEY_O, "iris.keybinds");
		...
		if (reloadKeybind.consumeClick()) {
			try {
				...
				if (minecraft.player != null) {
					minecraft.player.displayClientMessage(new TranslationTextComponent("iris.shaders.reloaded"), false);
				}
				...
				if (minecraft.player != null) {
					minecraft.player.displayClientMessage(new TranslationTextComponent("iris.shaders.reloaded.failure", Throwables.getRootCause(e).getMessage()).withStyle(TextFormatting.RED), false);
				}
			}
			...
			try {
				...
				reload();
				if (minecraft.player != null) {
					minecraft.player.displayClientMessage(new TranslationTextComponent("iris.shaders.toggled", config.areShadersEnabled() ? currentPackName : "off"), false);
				}
			} catch (Exception e) {
				logger.error("Error while toggling shaders!", e);

				if (minecraft.player != null) {
					minecraft.player.displayClientMessage(new TranslationTextComponent("iris.shaders.toggled.failure", Throwables.getRootCause(e).getMessage()).withStyle(TextFormatting.RED), false);
				}
	...
	public static DimensionId getCurrentDimension() {
		...
		if (level != null) {
			RegistryKey<net.minecraft.world.World> levelRegistryKey = level.dimension();

			if (levelRegistryKey.equals(net.minecraft.world.World.END)) {
				return DimensionId.END;
			} else if (levelRegistryKey.equals(net.minecraft.world.World.NETHER)) {
